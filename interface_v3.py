...
def main(): 
 ...
 if response == 4:
  ...
  if player_selection == 1:
   ...
   #choose information
   print("\n"+"----------------------------------")
   player_choice = int(input("Select Player ID: "))
   print("\n"+"----------------------------------")
   player_stat = int(input("Select the information you would like"+"\n"+"1. General Information"+"\n"+"2. Offensive Stats"+"\n"+"3. Defensive Stats"+"\n"))
   
   if player_stat == 1:
    print("\n"+"----------------------------------")
    cur.execute("SELECT p.f_name, p.l_name, t.team_name, p.nationality, p.salary, p.height, p.weight, p.dominant_foot FROM player p, team t WHERE p.team_id = t.team_id AND p.player_id = %s",(player_choice))
    for line in cur:
   ...
   if player_stat == 2:	
    cur.execute("SELECT p.f_name, p.l_name, o.goals, o.assists, o.playing_time_minutes FROM player p, offense o WHERE p.player_id = o.player_id AND p.player_id = %s",(player_choice))
    for line in cur:
     print("Player Name:",line[0],line[1])
   ...
   if player_stat == 3:
    cur.execute("SELECT p.f_name, p.l_name, d.goals_lost, d.tackles_made, d.fouls, d.saves, d.clean_sheet FROM player p, defense d WHERE p.player_id = d.player_id AND p.player_id = %s",(player_choice))
    for line in cur:
     print("Player Name:",line[0],line[1])
